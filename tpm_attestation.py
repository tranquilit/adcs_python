...
TPMA_OBJECT_FIXEDTPM = 2
TPMA_OBJECT_FIXEDPARENT = 16
TPMA_OBJECT_RESTRICTED = 65536
TPMA_OBJECT_DECRYPT = 131072
TPMA_OBJECT_SIGN = 262144
TPM2_ALG_RSA = 1
TPM2_ALG_ECC = 35
TPM2_ALG_SHA1 = 4
TPM2_ALG_SHA256 = 11
...
TPM2_ALG_ECDAA = 26
TPM2_ALG_SM2 = 27
TPM2_ALG_ECSCHNORR = 28
TPM2_ECC_NIST_P256 = 3
TPM2_ECC_NIST_P384 = 4
TPM2_GENERATED_VALUE = 4283712327
...
class _Reader:
    ...
    def u8(self) -> int:
        return self._consume(1, '>B')[0]

    def u16(self) -> int:
        return self._consume(2, '>H')[0]

    def u32(self) -> int:
        return self._consume(4, '>I')[0]

    def u64(self) -> int:
        return self._consume(8, '>Q')[0]
    ...
    def raw(self, n: int) -> bytes:
        if self._pos + n > len(self._d):
            raise TPMAttestationError(f'Parser overrun: need {n} bytes at offset {self._pos}, only {len(self._d) - self._pos} remaining')
...
def parse_tpms_attest(raw: bytes) -> AttestationData:
    ...
    attest = AttestationData(magic=magic, attest_type=attest_type, qualified_signer=qualified_signer, extra_data=extra_data, clock_info=full_clock_info, firmware_version=firmware_version, raw=raw)
    ...
    for root in trusted_roots:
        if root.subject == issuer:
            try:
                ...
                continue
    if not verified:
        raise TPMAttestationError("EK certificate does not chain to any trusted TPM manufacturer CA root. Add the manufacturer's CA to trusted_ek_roots.")
...
def _decrypt_cms_enveloped_data(content_info_der: bytes, recipient_cert_der: Optional[bytes], recipient_key) -> bytes:
    ...
    def _hash_from_name_or_oid(value):
        mapping = {'sha1': hashes.SHA1(), 'sha224': hashes.SHA224(), 'sha256': hashes.SHA256(), 'sha384': hashes.SHA384(), 'sha512': hashes.SHA512(), '1.3.14.3.2.26': hashes.SHA1(), '2.16.840.1.101.3.4.2.4': hashes.SHA224(), '2.16.840.1.101.3.4.2.1': hashes.SHA256(), '2.16.840.1.101.3.4.2.2': hashes.SHA384(), '2.16.840.1.101.3.4.2.3': hashes.SHA512()}
        if value in mapping:
            ...
            if oaep_params is not None:
                ...
                if p_source:
                    ...
                    if isinstance(psrc_params, bytes) and psrc_params:
                        label = psrc_params
            selected_cek = recipient_key.decrypt(encrypted_key, padding.OAEP(mgf=padding.MGF1(algorithm=mgf_hash_alg), algorithm=hash_alg, label=label))
        else:
...
def extract_microsoft_key_attestation_attributes_from_csr_der(csr_der: bytes) -> Optional[dict]:
    ...
    def _extract_first_attribute_value(values_field):
        ...
        try:
            ...
            pass
        return values_field
    result = {'ek_info_raw': None, 'aik_info_raw': None, 'attestation_statement_raw': None, 'attestation_blob_raw': None, 'ksp_name': None, 'ksp_name_raw': None, 'found': False}
    try:
        attr_oids = []
...
def _tpm_alg_to_hash(alg: int) -> str:
    return {TPM2_ALG_SHA1: 'sha1', TPM2_ALG_SHA256: 'sha256', TPM2_ALG_SHA384: 'sha384', TPM2_ALG_SHA512: 'sha512'}.get(alg, 'sha256')

def _tpm_alg_to_hash_obj(alg: int):
    return {TPM2_ALG_SHA1: hashes.SHA1(), TPM2_ALG_SHA256: hashes.SHA256(), TPM2_ALG_SHA384: hashes.SHA384(), TPM2_ALG_SHA512: hashes.SHA512()}.get(alg, hashes.SHA256())
...
class TPMPublicKey:
    ...
    def to_cryptography_public_key(self):
        if self.alg_type == TPM2_ALG_RSA:
            ...
            curve_map = {TPM2_ECC_NIST_P256: ec.SECP256R1(), TPM2_ECC_NIST_P384: ec.SECP384R1()}
    ...
    def _marshal(self) -> bytes:
        ...
        if self.alg_type == TPM2_ALG_RSA:
            buf += struct.pack('>HHHhI', TPM2_ALG_NULL, TPM2_ALG_NULL, self.rsa_key_bits, TPM2_ALG_NULL, self.rsa_exponent)
            buf += _tpm2b(self.rsa_modulus)
        elif self.alg_type == TPM2_ALG_ECC:
            buf += struct.pack('>HHHH', TPM2_ALG_NULL, TPM2_ALG_NULL, self.ecc_curve, TPM2_ALG_NULL)
...
def parse_tpmt_public(raw: bytes) -> TPMPublicKey:
    ...
    obj_attr = r.u32()
    auth_policy = r.tpm2b()
    result = TPMPublicKey(alg_type=alg_type, name_alg=name_alg, object_attr=obj_attr, auth_policy=auth_policy)
...
def _verify_tpm_signature(data: bytes, sig_raw: bytes, pub_key: TPMPublicKey):
    ...
    if sig_alg == TPM2_ALG_RSASSA:
        ...
        sig_bytes = r.tpm2b()
        try:
            crypto_key.verify(sig_bytes, data, padding.PSS(mgf=padding.MGF1(hash_obj), salt_length=padding.PSS.MAX_LENGTH), hash_obj)
        except InvalidSignature:
...
def _check_aik_attributes(aik: TPMPublicKey):
    ...
    if aik.object_attr & required != required:
        raise TPMAttestationError(f'AIK must have RESTRICTED|SIGN attributes set. Got TPMA_OBJECT={aik.object_attr:#010x}')
...
def _kdfe(hash_alg: int, z: bytes, label: str, party_u_info: bytes, party_v_info: bytes, bits: int) -> bytes:
    ...
    if extra_bits:
        out = bytes([out[0] & 255 >> extra_bits]) + out[1:]
    return out
...
def tpm2_make_credential(ek_pub, object_name: bytes, credential_value: bytes, ek_name_alg: int=TPM2_ALG_SHA256, sym_bits: int=128) -> tuple[bytes, bytes]:
    ...
    if isinstance(ek_pub, rsa.RSAPublicKey):
        ...
        encrypted_secret = ek_pub.encrypt(seed, padding.OAEP(mgf=padding.MGF1(algorithm=_tpm_alg_to_hash_obj(ek_name_alg)), algorithm=_tpm_alg_to_hash_obj(ek_name_alg), label=label))
    elif isinstance(ek_pub, ec.EllipticCurvePublicKey):
        ...
        ephemeral_pub_bytes = ephemeral_pub.public_bytes(serialization.Encoding.X962, serialization.PublicFormat.UncompressedPoint)
        ek_pub_bytes = ek_pub.public_bytes(serialization.Encoding.X962, serialization.PublicFormat.UncompressedPoint)
        coord_size = (ek_pub.key_size + 7) // 8
        ephem_x = ephemeral_pub_bytes[1:1 + coord_size]
    ...
    outer_hmac = hmac.new(hmac_key, encrypted_credential + object_name, ek_hash_name).digest()
...
def _parse_microsoft_key_attestation_statement(blob: bytes) -> dict:
    ...
    return {'offset': pos, 'version': version, 'platform': platform, 'header_size': header_size, 'id_binding': blob[start:end_id], 'key_attestation': blob[end_id:end_key], 'aik_opaque': blob[end_key:end_aik]}
...
def _verify_ek_only(bundle: TPMAttestationBundle, trusted_ek_roots: list) -> AttestationResult:
    ...
    logger.info('EK_ONLY attestation OK — manufacturer=%s subject=%s', manufacturer, ek_cert.subject.rfc4514_string())
    return AttestationResult(success=True, mode='EK_ONLY', message='EK certificate verified against trusted TPM CA roots', ek_cert=ek_cert, manufacturer=manufacturer)

def _verify_aik_full(bundle: TPMAttestationBundle, trusted_ek_roots: list, expected_nonce: Optional[bytes], require_fixed_tpm: bool, require_fixed_parent: bool, require_restricted: bool) -> AttestationResult:
    if not bundle.ek_cert_der:
    ...
    _verify_tpm_signature(data=bundle.attest_raw, sig_raw=bundle.attest_sig_raw, pub_key=aik_pub)
    ...
    logger.info('AIK_FULL attestation OK — manufacturer=%s firmware=%016x', manufacturer, attest.firmware_version)
    return AttestationResult(success=True, mode='AIK_FULL', message='AIK attestation verified (EK chain + quote signature + nonce)', ek_cert=ek_cert, aik_public_key=aik_pub, is_fixed_tpm=bool(aik_pub.object_attr & TPMA_OBJECT_FIXEDTPM), is_fixed_parent=bool(aik_pub.object_attr & TPMA_OBJECT_FIXEDPARENT), is_restricted=bool(aik_pub.object_attr & TPMA_OBJECT_RESTRICTED), firmware_version=attest.firmware_version, manufacturer=manufacturer)

def _verify_certify(bundle: TPMAttestationBundle, trusted_ek_roots: list, expected_nonce: Optional[bytes], require_fixed_tpm: bool, require_fixed_parent: bool, require_restricted: bool) -> AttestationResult:
    if not bundle.ek_cert_der:
    ...
    if attest.attest_type != TPM2_ST_ATTEST_CERTIFY:
        raise TPMAttestationError(f'Expected ST_ATTEST_CERTIFY ({TPM2_ST_ATTEST_CERTIFY:#06x}), got {attest.attest_type:#06x}')
    if expected_nonce is not None:
        if not hmac.compare_digest(attest.extra_data, expected_nonce):
            raise TPMAttestationError('Nonce mismatch — possible replay attack')
    _verify_tpm_signature(data=bundle.attest_raw, sig_raw=bundle.attest_sig_raw, pub_key=aik_pub)
    ...
    if attest.certified_name != expected_name:
        raise TPMAttestationError("certifiedName in TPMS_ATTEST does not match the certified key's computed TPM name — key mismatch or tampered attestation")
    _check_key_policy(cert_key, require_fixed_tpm, require_fixed_parent, require_restricted)
    manufacturer = _extract_tpm_manufacturer(ek_cert)
    logger.info('CERTIFY attestation OK — manufacturer=%s firmware=%016x', manufacturer, attest.firmware_version)
    return AttestationResult(success=True, mode='CERTIFY', message='TPM2_Certify attestation verified (EK chain + AIK signature + key name match)', ek_cert=ek_cert, aik_public_key=aik_pub, certified_key_obj=cert_key, is_fixed_tpm=bool(cert_key.object_attr & TPMA_OBJECT_FIXEDTPM), is_fixed_parent=bool(cert_key.object_attr & TPMA_OBJECT_FIXEDPARENT), is_restricted=bool(cert_key.object_attr & TPMA_OBJECT_RESTRICTED), firmware_version=attest.firmware_version, manufacturer=manufacturer)

def verify_tpm_attestation(bundle: TPMAttestationBundle, trusted_ek_roots: list, expected_nonce: Optional[bytes]=None, require_fixed_tpm: bool=True, require_fixed_parent: bool=True, require_restricted: bool=True) -> AttestationResult:
    try:
        if bundle.mode == 'EK_ONLY':
            return _verify_ek_only(bundle, trusted_ek_roots)
        elif bundle.mode == 'AIK_FULL':
            return _verify_aik_full(bundle, trusted_ek_roots, expected_nonce, require_fixed_tpm, require_fixed_parent, require_restricted)
        elif bundle.mode == 'CERTIFY':
            return _verify_certify(bundle, trusted_ek_roots, expected_nonce, require_fixed_tpm, require_fixed_parent, require_restricted)
        else:
    ...
    try:
        for ext in csr.extensions:
            if ext.oid.dotted_string == OID_TPM_ATTESTATION_BUNDLE:
                ...
                return TPMAttestationBundle(mode=blob.get('mode', 'AIK_FULL'), ek_cert_der=_b64_or_none(blob.get('ek_cert')), aik_pub_raw=_b64_or_none(blob.get('aik_pub')), attest_raw=_b64_or_none(blob.get('attest')), attest_sig_raw=_b64_or_none(blob.get('sig')), certified_key_raw=_b64_or_none(blob.get('cert_key')), nonce=_b64_or_none(blob.get('nonce')))
    except Exception as exc:
...
def extract_ek_pub_from_decrypted_ek_info(decrypted_ek_info: bytes, *, embedded_certificates_der=None, ek_cert_der: Optional[bytes]=None):
...
def build_ms_challenge_wrapper_value(*, encryption_algorithm_oid: str, aik_info_hash: bytes | None, ksp_name: str, tach_blob: bytes, inner_body_part_id: int=1) -> bytes:
    ...
    if aik_info_der is not None:
        attr_aik = MsWrappedAttr({'oid': a_core.ObjectIdentifier(OID_ENROLL_CAXCHGCERT_HASH), 'values': [_as_any_from_der(aik_info_der)]})
    attr_ksp = MsWrappedAttr({'oid': a_core.ObjectIdentifier(OID_ENROLL_KSP_NAME), 'values': [_as_any_from_der(a_core.BMPString(ksp_name).dump())]})
    attr_tach = MsWrappedAttr({'oid': a_core.ObjectIdentifier(OID_ENROLL_ATTESTATION_CHALLENGE), 'values': [_as_any_from_der(a_core.OctetString(tach_blob).dump())]})
...
def build_adcs_like_control_sequence(*, request_id: int, encryption_algorithm_oid: str, aik_info_hash: bytes | None, tach_blob: bytes, ksp_name: str='Microsoft Platform Crypto Provider', pend_time=None) -> TaggedAttributes:
    ...
    ms_wrapper_der = build_ms_challenge_wrapper_value(encryption_algorithm_oid=encryption_algorithm_oid, aik_info_hash=aik_info_hash, ksp_name=ksp_name, tach_blob=tach_blob, inner_body_part_id=1)
    ctrl_pending = TaggedAttribute({'bodyPartID': BodyPartID(1), 'attrType': a_core.ObjectIdentifier(OID_CMC_STATUS_INFO), 'attrValues': [_as_any_from_der(pending_der)]})
    ctrl_ms_challenge = TaggedAttribute({'bodyPartID': BodyPartID(2), 'attrType': a_core.ObjectIdentifier(OID_MS_CMC_CHALLENGE_WRAPPER), 'attrValues': [_as_any_from_der(ms_wrapper_der)]})
...
def build_microsoft_attestation_challenge_pki_response(*, request_id: int, ca_exchange_chain_der, encryption_algorithm_oid: str, aik_info_hash: bytes | None, tach_blob: bytes, pend_time=None) -> dict:
    control_seq = build_adcs_like_control_sequence(request_id=request_id, encryption_algorithm_oid=encryption_algorithm_oid, aik_info_hash=aik_info_hash, tach_blob=tach_blob, ksp_name='Microsoft Platform Crypto Provider', pend_time=pend_time)
...
def _sign_cmc_pki_response_python(*, pki_response_der: bytes, signer_cert_pem, signer_key_pem, extra_chain_pems=None, extra_certs_der=None) -> bytes:
    ...
    signer_info = a_cms.SignerInfo({'version': 'v1', 'sid': a_cms.SignerIdentifier({'issuer_and_serial_number': a_cms.IssuerAndSerialNumber({'issuer': signer_asn1.issuer, 'serial_number': signer_asn1.serial_number})}), 'digest_algorithm': a_cms.DigestAlgorithm({'algorithm': 'sha256', 'parameters': a_core.Null()}), 'signed_attrs': signed_attrs, 'signature_algorithm': a_cms.SignedDigestAlgorithm({'algorithm': 'rsassa_pkcs1v15', 'parameters': a_core.Null()}), 'signature': signature})
    sd = a_cms.SignedData({'version': 'v3', 'digest_algorithms': [a_cms.DigestAlgorithm({'algorithm': 'sha256', 'parameters': a_core.Null()})], 'encap_content_info': {'content_type': OID_ID_CCT_PKI_RESPONSE, 'content': a_cms.ParsableOctetString(pki_response_der)}, 'certificates': OrderedCertificateSet(cert_choices), 'signer_infos': [signer_info]})
    ci = a_cms.ContentInfo({'content_type': OID_ID_SIGNED_DATA, 'content': sd})
    out = ci.dump()
    if b'\x06\t*\x86H\x86\xf7\r\x01\x07\x01' in out and b'\x06\x08+\x06\x01\x05\x05\x07\x0c\x03' not in out:
...
def build_and_sign_microsoft_attestation_challenge(*, request_id: int, ek_pub, ca_exchange_chain_der, encryption_algorithm_oid: str, aik_info_hash: bytes | None, signer_cert_pem, signer_key_pem, signer_chain_pems=None, secret: bytes | None=None, openssl_bin: str='openssl', aik_pub_raw: bytes | None=None, attestation_blob_raw: bytes | None=None) -> dict:
    ...
    tach_blob = _make_tach_blob(secret=secret, ek_pub=ek_pub, aik_name=aik_name, attestation_blob_raw=attestation_blob_raw, ek_name_alg=ek_name_alg)
    built = build_microsoft_attestation_challenge_pki_response(request_id=request_id, ca_exchange_chain_der=ca_exchange_chain_der, encryption_algorithm_oid=encryption_algorithm_oid, aik_info_hash=aik_info_hash, tach_blob=tach_blob)
    built['request_id'] = int(request_id)
    built['effective_request_id'] = int(request_id)
    signed_pkcs7_der = _sign_cmc_pki_response_python(pki_response_der=built['response_body_der'], signer_cert_pem=signer_cert_pem, signer_key_pem=signer_key_pem, extra_chain_pems=signer_chain_pems, extra_certs_der=ca_exchange_chain_der)
    logger.debug('TPM response_body=%d bytes signed_pkcs7=%d bytes tach=%d bytes', len(built['response_body_der']), len(signed_pkcs7_der), len(tach_blob))
    built['secret'] = secret
